...
void motor_zero::record_position(){
    ...
    std::vector<std::vector<MotorCmd>> legCmds(leg_num, std::vector<MotorCmd>(bus_motor_num));
    std::vector<std::vector<MotorData>> legDatas(leg_num, std::vector<MotorData>(bus_motor_num));
    ...
    while(!exit_thread){
        for (int leg = 0; leg < leg_num; ++leg) {
            for (int j = 0; j < bus_motor_num; ++j) {
                legCmds[leg][j].id = leg * joint_num + j;
                legCmds[leg][j].mode = 1;
                legCmds[leg][j].K_P = 0;
                legCmds[leg][j].K_W = 0;
                legCmds[leg][j].Pos = 0;
                legCmds[leg][j].W = 0;
